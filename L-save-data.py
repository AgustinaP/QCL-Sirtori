...
def PlotBandZProfile_field(ax,structure,conduction_band = True,valence_band = False,n = 500):
    ...
    with open("3p-EF"+str(EF_)+'-X1'+str(X1[i])+'-pozo.pickles', 'wb') as f:
...
for i in range(len(X1)):
    ...
    structure.Load("3p-EF"+str(EF_)+'-X1'+str(X1[i])+".str")
    EF = - structure.GetParameterDouble('FIELD') * 1e7 /1000 #kV/cm
    
    fig, ax = plt.subplots(figsize=(8, 6))
    PlotBandZProfile_field(ax,structure,conduction_band = True,valence_band = False,n = 2000)
    plt.show()
    ### solve p1b ###
    ...
    print("p1b solved to ",X1[i])
    ### Save with pickle ###
    with open("3p-EF"+str(EF_)+'-X1'+str(X1[i])+'-p1b.pickles', 'wb') as handle:
    ...
    ### Save with pickle ###
    with open("3p-EF"+str(EF_)+'-X1'+str(X1[i])+'-e2b.pickles', 'wb') as handle:
