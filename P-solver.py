...
for i in range(len(periodos)):
    ...
    structure.Load(str(periodos[i])+"p-EF"+str(EF_)+".str")
    X1_1p = 0.33
    EF = - structure.GetParameterDouble('FIELD') * 1e7 /1000 #kV/cm
    if i == 3:
        fig, ax = plt.subplots(figsize=(8, 6))
        PlotBandZProfile_field(ax,structure,conduction_band = True,valence_band = False,n = 500*i+500)
    ### solve p1b ###
    p1b_solver_cond = pythera.edifi.SolverP1b(pythera.edifi.BandTypes.Conduction, structure, 300*i+350,5*i+30)
    ...
    ### Save with pickle ###
    with open(str(periodos[i])+"p-EF"+str(EF_)+'-p1b.pickles', 'wb') as handle:
        pickle.dump([z,dens, p1b_energies_cond], handle, protocol=pickle.HIGHEST_PROTOCOL)

    ### solve e2b ###
    e2b_solver = pythera.edifi.SolverE2b(False, structure, 300*i+350,10*i+30)
    e2b_solver.Solve()
    ...
    ### Save with pickle ###
    with open(str(periodos[i])+"p-EF"+str(EF_)+'-e2b.pickles', 'wb') as handle:
