...
for i in range(len(TEMPERATURE)):
    ...
    structure.Load("3p-EF"+str(EF_)+'-T'+str(TEMPERATURE[i])+".str")
    X1_1p = 0.33
    EF = - structure.GetParameterDouble('FIELD') * 1e7 /1000 #kV/cm
    if i == 0:
        fig, ax = plt.subplots(figsize=(8, 6))
        PlotBandZProfile_field(ax,structure,conduction_band = True,valence_band = False,n = 1500)
    ### solve p1b ###
    ...
    print("p1b solved to ",TEMPERATURE[i]," K")
    ### Save with pickle ###
    with open("3p-EF"+str(EF_)+'-T'+str(TEMPERATURE[i])+'-p1b.pickles', 'wb') as handle:
        pickle.dump([z,dens, p1b_energies_cond], handle, protocol=pickle.HIGHEST_PROTOCOL)
    print("p1b saved to ",TEMPERATURE[i]," K")
    ...
    print("e2b solved to ",TEMPERATURE[i]," K")
    ### Save with pickle ###
    with open("3p-EF"+str(EF_)+'-T'+str(TEMPERATURE[i])+'-e2b.pickles', 'wb') as handle:
        pickle.dump([z,dens,e2b_energies], handle, protocol=pickle.HIGHEST_PROTOCOL)
    print("e2b saved to ",TEMPERATURE[i]," K")